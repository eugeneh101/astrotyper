...
describe('GameEngine Input Handling', () => {
  ...
  test('Pressing 1 in BRANCHING should select branch and set to WARPING', () => {
    ...
    engine.setNextBranches([
      {
        action_summary: 'Evasive',
        full_narrative: 'Evasive maneuvers engaged.',
      },
  ...
  test('onDefeat should fire once, and reset correctly after replayLevel', () => {
    ...
    // Manually trigger Game Over state
    engine.state = 'GAMEOVER';
    
    // GameEngine uses a 3.0 second death animation timer.
    // We bypass the private visibility of update to simulate frames.
    (engine as any).update(3.1);

    // Should have fired exactly once
    expect(mockOnDefeat).toHaveBeenCalledTimes(1);

    // Subsequent updates in GAMEOVER state should NOT fire it again
    (engine as any).update(0.1);
    expect(mockOnDefeat).toHaveBeenCalledTimes(1);

    // Call replayLevel which resets flags
    engine.replayLevel(1, 100, '', 'New wave text.');

    // State should be back to PLAYING and flags reset
    expect(engine.state).toBe('PLAYING');
    expect(engine.defeatFired).toBe(false);
