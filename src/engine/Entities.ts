...
export class Enemy {
  ...
  angle: number = 0;
  hasFiredCharge?: boolean;
  type: 'kamikaze' | 'shooter' | 'boss_target' | 'mini_bio' | 'scrambler' = 'kamikaze';
  targetStopY: number = 0;
  strafePhase: number = 0;
  strafeSpeed: number = 100;
  chargeTime: number = 0;
  maxChargeTime: number = GAME_CONFIG.ENEMY_SHOOTER_CHARGE_TIME;
  laserAlpha: number = 0;
  scatterParticles: {
    x: number;
    y: number;
  ...
  constructor(
    ...
    this.type = type;
    if (
      (this.type === 'shooter' || this.type === 'scrambler') &&
      this.typedChars === 0 &&
      !this.isDead
    ) {
  ...
  update(
    ...
    if (this.type === 'kamikaze' || this.type === 'mini_bio') {
      // The enemy's nose extends 15px forward from its center.
      if (dist < GAME_CONFIG.PLAYER_SHIELD_RADIUS + 15 && !this.hitShield) {
        this.hitShield = true;
    ...
    } else if (this.type === 'shooter' || this.type === 'scrambler') {
  ...
  draw(
    ...
    if ((this.type === 'shooter' || this.type === 'scrambler') && this.laserAlpha > 0) {
      // Draw massive laser beam or scrambler beam
      ctx.save();
    ...
    if (this.type === 'kamikaze') {
      ...
      const isScrambler = this.type === 'scrambler';
      ctx.shadowColor = isScrambler ? '#ffff00' : '#aa00ff';
      ...
      ctx.fillStyle = isScrambler ? '#1a1a00' : '#14051a'; // Dark yellow-ish or purple-ish
      ctx.fill();
      ctx.strokeStyle = isScrambler ? '#ffff00' : '#7700ff';
      ctx.lineWidth = 1;
      ctx.stroke();

      // Cockpit (sinister glowing slit)
      ctx.beginPath();
      ctx.moveTo(-12, -7);
      ctx.lineTo(12, -7);
      ctx.lineTo(8, -4);
      ctx.lineTo(-8, -4);
      ctx.closePath();
      ctx.fillStyle = isScrambler ? '#00ffcc' : '#ffaa00';
      ctx.fill();

      // Charge Indicator Orb
      if (!isGameOver) {
        const chargeRatio = this.chargeTime / this.maxChargeTime;
        const pulse = Math.sin(Date.now() / 80) * 0.15;

        ctx.beginPath();
        // Scrambler uses a square core, Shooter uses a circle
        if (isScrambler) {
          const size = 4 + chargeRatio * 14;
          ctx.rect(-size/2, 15 - size/2, size, size);
        } else {
          ctx.arc(0, 15, 4 + chargeRatio * 14, 0, Math.PI * 2);
        }
        
        ctx.fillStyle = isScrambler 
          ? `rgba(255, 255, 0, ${0.4 + chargeRatio * 0.6 + pulse})`
          : `rgba(255, 60, 0, ${0.4 + chargeRatio * 0.6 + pulse})`;
        ctx.shadowBlur = 15 + chargeRatio * 40;
        ctx.shadowColor = isScrambler ? '#ffff00' : '#ff2200';
        ctx.fill();

        // Super hot core
        ctx.beginPath();
        if (isScrambler) {
          const coreSize = 2 + chargeRatio * 10;
          ctx.rect(-coreSize/2, 15 - coreSize/2, coreSize, coreSize);
        } else {
        ...
        ctx.moveTo(0, 25);
        ctx.lineTo(0, 800);
        ctx.strokeStyle = isScrambler 
          ? `rgba(0, 255, 204, ${0.15 + chargeRatio * 0.6})`
          : `rgba(255, 0, 50, ${0.15 + chargeRatio * 0.6})`;
        ctx.lineWidth = 1 + chargeRatio * 4;
        ctx.setLineDash([5, Math.max(2, 25 - chargeRatio * 23)]); // Dashes speed up
        ctx.stroke();
        ctx.setLineDash([]);
      }
    }
