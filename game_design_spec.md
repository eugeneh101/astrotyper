...
*   **Visuals & UI:** Top-Down 2D HTML5 Canvas. Cool blue/green holographic UI.
    ...
    *   *Level Transitions & Backgrounds:* Each level features a distinct, auto-scrolling parallax background to signify narrative progression (e.g., Deep Space -> Atmosphere -> Cityscape -> Reactor Core). Between levels, the player's ship performs a cinematic "Warp" animation (stretching and blasting upward in a flash of light) to seamlessly load the next environment.
*   **Enemies & Combat:** The visual design of the enemies is *not* tied to the length of the word they carry. Enemies are drawn as sleek, dark multi-layered dreadnoughts with swept-back wings, glowing red cockpit cores, neon wing-vents, and flickering orange/white gradient exhaust trails.
    ...
    *   **Level 1:** 1 Enemy Type (Kamikaze only).
    *   **Level 2:** 2 Enemy Types (Kamikaze + Shooter A).
    *   **Level 3:** 
    ...
    *   **Level 4 (Fast/Very Fast Only):** The Final Boss.
    *   **The Final Boss Mechanic:** At the end of the final level (Level 3 for < 70 WPM, Level 4 for >= 70 WPM), a massive "Boss" ship spawns. Instead of each word being a separate ship, the entire final sentence (or two) of the story belongs to this single Boss. As the player types the words, they systematically deal damage to the Boss's health pool to win the game!
        *   **Dynamic Encounters:** The boss encountered is dynamically selected based on the player's WPM. Slow/Medium typers face the **Cybernetic Dreadnought** (Tech mechanics: QTE Core Obliteration Laser dealing 5 damage/sec, Turret Bullet Hell, 5 distinct erratic and sweeping strafe patterns, massive stealth-bomber chassis with pulsing cyan exhaust). Fast/Very Fast typers face the **Abyssal Bio-Entity** (Organic mechanics: Spinning Toxic Danmaku, 4-second QTE Beam Attack dealing 10 damage/sec, Black Hole Spaghettification death). For both bosses, taking beam damage strictly halts the player's passive shield regeneration. The QTE warning UI is clustered to the right side of the boss to prevent vertical screen clipping.
        *   **The Standoff:** During the Boss fight, the scrolling/forward movement stops. It is a static standoff where the Boss and the player's ship are duking it out face-to-face. The Boss fires multiple different projectiles that the player must manage while frantically typing the final sentence. The text for the Boss is rendered exclusively in the bottom HUD text box, rather than hovering over the boss sprite itself, to ensure readability amidst the bullet hell.
        *   **Deflector Shield Overcharge:** To prevent instant death and offer a strategic defensive layer, the player builds up a "Deflector Charge" meter by typing words perfectly across *all* waves (not just the boss wave). Every completed word adds to the charge. Once full, an Energy Deflector Shield aura (a glowing cyan ring) expands outward around the player's ship for 5 seconds, vaporizing all incoming bullet hell projectiles AND boss beams without taking damage. After it expires, the player can instantly begin charging it again (there is no cooldown).
...
*   **Progression & Health (Regenerating Shields):** The player has a Shield/Life bar (not discrete hearts) and can take some damage during a level. If the player takes damage from a Kamikaze or a Shooter's Charge Laser, the shield depletes. However, if the player avoids taking damage for 3 consecutive seconds, their ship's shields will automatically begin to recharge (healing at a slow rate of 2 points per second) until they are struck again. There are no other power-ups to keep the focus on the narrative and survival.
...
*   **Pacing & The Threat of Death:** The game dynamically calculates the player's WPM, but to maintain intense suspense, it does not just match it perfectly. 
    ...
    *   **The Climax:** After Level 2, the game locks in the highest established pace for the Climax. The game will *not* slow down if the player gets fatigued.
*   **Combat Loop (Zero-Latency Architecture):**
    ...
    *   **The Death Blossom:** When the player types the very final word of the Boss fight (the killing blow), their ship performs a 360-degree spin and unleashes a blinding barrage of lasers in all directions. This instantly destroys all remaining bullet-hell projectiles and obliterates the Boss in a cinematic explosion.
    1.  A "Level/Wave" begins. The player is typing the story option they selected in the previous level (about 10 sentences long, taking 1-2 minutes).
    2.  **Background Prefetching (The 70% Trigger):** To accurately measure the player's Words Per Minute (WPM), the game tracks their typing speed throughout the level. When the player reaches **70% completion** of the current wave, the frontend calculates their running average WPM and fires **one** asynchronous API request to the Python Agent. This provides an accurate WPM to the Agent while still giving the LLM enough time to generate the next choices before the player finishes the final 30% of the text.
    ...
    4.  **Path Selection:** When the wave is cleared, the 3 `action_summary` choices appear alongside real-time WPM and Words Typed metrics. The player types out the summary (e.g. "Evasive maneuvers") to select that path. The `full_narrative` of that path instantly becomes the text for the next wave!
...
*   **No 4th Wall Breaks:** The LLM will be instructed to act strictly as a gritty/epic sci-fi narrator. It must never reference the game mechanics, the player typing, or the concepts of "levels" or "WPM".
*   **Dynamic Archetype Divergence:** To guarantee the 3 choices feel like meaningful, distinct branching paths, the game engine will randomly select **3 archetypes** from a predefined pool and pass them to the ADK Agent for each level. The LLM must map its 3 generated choices to these specific archetypes. The pool includes:
...
*   **Level 1 Baseline (The Calibration Level):** Level 1 (The Beginning) will always target **~90 words**. At the 70% mark, the game calculates WPM. The ADK Agent uses this to generate Level 2 (The Middle) with a target word count based on the player's tier.
*   **The Level 2 Decision Point (The 4 Tiers):** The total number of levels is not locked in until the 70% mark of Level 2. The game calculates the *new* running average WPM to determine the structure for Level 3:
    *   **Slow (20-39 WPM):** 3 levels total. Level 3 is the combined **Climax & Ending** (~45 words per level for R2 and R3). Game ends after Level 3. Total Words: ~180.
    *   **Medium (40-69 WPM):** 3 levels total. Level 3 is the combined **Climax & Ending** (~90 words per level for R2 and R3). Game ends after Level 3. Total Words: ~270.
    *   **Fast (70-89 WPM):** 4 levels total. Level 3 is the **Climax**. Level 4 is the **Ending** (~90 words per level for R2, R3, R4). Total Words: ~360.
    *   **Very Fast (90+ WPM):** 4 levels total. Level 3 is the **Climax**. Level 4 is the **Ending** (~120 words per level for R2, R3, R4). Total Words: ~450.
...
*   **The End Screen (Victory or Defeat):** 
    *   *If the player wins*, the game engine enters a `VICTORY` state and halts rendering of the ship/HUD. A sleek React DOM overlay fades in, presenting the final Words Per Minute (WPM), total word count, and a massive scrollable text box (which defaults to scrolling to the top) containing their entire unique generated story, beautifully separated into paragraphs for each level.
    *   *If the player dies* (life bar hits zero), the game enters `GAMEOVER` but allows the enemies, bosses, and lasers to continue animating and flying for exactly 3 seconds to let the player watch their own destruction. The game then proceeds directly to the "Mission Failed" screen, which concatenates previous story history, the final typed characters, terminal fatal error codes, and a glowing, animated `[ TRANSMISSION LOST ]` UI element.
    *   *Replay Level:* The Mission Failed screen features a "Replay Level" button. Instead of restarting the entire game, this button resets the player's health and restarts the *exact same wave* they died on, preserving their story and WPM up to that point.
