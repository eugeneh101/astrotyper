...
*   **Pure Agentic Architecture:** I didn't just plug in an API; I engineered an agentic system. I adhered strictly to best practices, entirely rejecting "vibe coding" (which is essentially hope-driven development). Leveraging the Google GenAI SDK and the Gemini API, the backend Game Master is strictly governed by stateless memory graphs, robust Pydantic data bounds, and rigid system prompts that force the LLM to behave like a deterministic game engine rather than a stochastic chatbot.
*   **Centralized Specs & Framework Rules:** Instead of hallucinating features, my AI agent was anchored to a strict [`game_design_spec.md`](https://github.com/eugeneh101/astrotyper/blob/master/game_design_spec.md) source of truth and global [`AGENTS.md`](https://github.com/eugeneh101/astrotyper/blob/master/.agents/AGENTS.md) constraints to ensure architectural consistency.
*   **Custom Agent Skills:** I developed [agent skills](https://github.com/eugeneh101/astrotyper/tree/master/.agents/skills) (like `/git-commit` and `/stride_threat_model`) that empowered the AI to autonomously navigate complex DevOps and testing workflows.
    *   **STRIDE Threat Modeling:** I treated the GenAI integration as a security surface and created `/stride_threat_model` skill to generate a systematic [`threat_model.md`](https://github.com/eugeneh101/astrotyper/blob/master/threat_model.md) to mitigate prompt injection and enforce rigid schema validation.
...
*   **The "LLM-as-a-Judge" QA Suite:** How do you test an AI that can say anything? I built a pure Python offline evaluation suite ([`run_evals.py`](https://github.com/eugeneh101/astrotyper/blob/master/tests/eval/run_evals.py)) that uses a secondary LLM judge to deterministically grade the Game Master against negative constraints. It ensures the AI *never* breaks the 4th wall, never hallucinates game mechanics, and always hits exact word count targets.
